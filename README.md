...
### Representação Interna do Estado do Jogo (158w)
...
- O **jogador** toma os valores 'player1' ou 'player2', e é lhe atribuído um nome dado pelo utilizador no caso de ser um jogador humano, ou 'Computer' no caso de ser um computador. Para os computadores, é também atribuído um nível de dificuldade, 'easy' ou 'hard'.
...
<img src='src/images/board_init.png'><br>
**Tabuleiro intermédio** <br>
<img src='src/images/board_mid.png'><br>
**Tabuleiro final** <br>
<img src='src/images/board_final.png'><br>



### Validação e Execução de Jogadas
A forma como se valida e executa cada jogada está dependente do seu tipo. Para jogadas do tipo "**Adicionar peão**", utilizamos o predicado `place_pawn/5` para verificar se a célula escolhida está vazia e se o jogador ainda tem peões disponíveis. Se estas condições se verificarem, é chamado o predicado `place_tower/5`, que adiciona um peão do respetivo jogador à célula escolhida e retorna o novo tabuleiro (`NewBoard`). Se alguma das condições de validação falhar, será impressa uma mensagem de feedback a explicar o motivo.
```prolog
...
```
Para jogadas dos tipos "**Mover torre**" e "**Separar torre**", devido à complexidade do movimento das peças, foi necessário criar um predicado `valid_moves/4`que calcula todas as jogadas válidas para a peça indicada pelo utilizador e pede para este escolher uma delas, garantindo assim que a jogada está sempre válida. 
```prolog
...
```
Esse cálculo das jogadas válidas tem em conta os movimentos possíveis para o tipo de peça escolhida (semelhantes ao xadrez, mas apenas para casas já ocupadas com outras torres), com o predicado `valid_piece_movement/6` e se esse movimento não resulta numa vitória direta do oponente (`check_possible_tower/7`).
...
De modo a respeitar a "KO rule", também se verifica se a nova jogada não desfaz a jogada anterior do openente. Para isso, é guardado o estado do jogo anterior, que depois é comparado com o estado atual.
...
```
No caso de **Separar torre**, esse predicado é o `separate_tower/7`, que separa a torre em (`X`, `Y`) em duas torres, uma com os `N` discos do topo, que será movida para (`NewX`, `NewY`) e outra com o resto dos discos, que ficará na célula original (`X`, `Y`).
```prolog
...
Definimos duas dificuldades para o computador: *easy* e *hard*. 

Para a modo fácil, o computador escolhe uma jogada aleatória da lista de todas as jogadas válidas. Traduz essa jogada para um tabuleiro novo através do predicado `translate_move/3` e verifica se essa jogada não desfaz a jogada anterior do oponente.
...
Para a modo difícil, utilizamos um algoritmo *greedy* que utiliza a valorização dos tabuleiros de jogo para implementar um algoritmo Minimax com dois níveis de profundidade. 
Começa por traduzir cada jogada da lista de jogadas válidas para o respetivo tabuleiro e avalia-o através do predicado `value/3`. De seguida, testa todas as respostas possíveis por parte do oponente, avaliando-as também. Calcula a diferença (Delta) entre os valores obtidos para a jogada do jogador e cada uma das respostas, guardando o menor Delta, ou seja, o que representa a melhor resposta do oponente. Por fim, de todas as jogadas possíveis escolhe aquela que tem um maior delta associado, garantindo assim que o computador escolhe a jogada que lhe dá mais vantagem, tendo em conta que o adversário também jogará da melhor forma possível.
...
Todas as jogadas são também corretamente validadas e é impedido que a jogada escolhida pelo jogador atual desfaça a jogada anterior do oponente. O estado do jogo anterior é guardado e, desta forma, não ocorrerá um ciclo infinito.
